	...
	private String landscapeLayout(FullRecipe recipe) {
		...
		if (recipe.ingredientGroups != null) {
			...
			while (igs.hasNext()) {
				...
				Iterator<Ingredient> is = ig.ingredients.iterator();
				while (is.hasNext()) {
					Ingredient i = is.next();

					String text = i.getIngredient();
					text = performRegexes(text);

					if (i.hasReference()) {
						ingredients = ingredients + "<li><a href='" +
							Global.httpToRecipeConvert(i.getReferenceUri()) +
							"'>" +
							text + "</a></li>";
					}
					else {
						ingredients = ingredients + "<li>" + text + "</li>";
					}
				}
